...
-- (human-service, originally apollo-service) onto the canonical lead, so the
-- send chain (email-gateway-service → instantly-service) can schedule cold email
-- in the recipient's local business hours. Nullable + backward-compatible: absent
