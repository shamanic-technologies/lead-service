/**
 * A sales funnel is an ORDERED sequence of steps, and what is known about one of its steps
 ...
 *     Paid client        outcome
 *
 * Nobody attends a meeting that was never booked. Two rules follow from the funnel's order, and
 ...
 * way".
 *
 * "Before" and "after" mean nothing without knowing WHICH funnel the lead is on: a campaign
 * selling meetings off replies runs reply -> booked -> attended -> paid, one selling off the
 * website runs visit -> signup -> paid. So the order is per FUNNEL. The funnel is stated by the
 * CAMPAIGN (campaign-service owns `funnelKey`, and lead rows carry their campaign), and the
 * catalogue below is the funnel key -> ordered steps mapping expressed in THIS service's step
 * vocabulary.
 *
 ...
 *
 * A funnel's steps here deliberately contain ONLY steps a statement can be made about.
 * `conversation_reply` and `ad_click` start two of the catalogue's funnels and are not conversion
 * outcomes here, so they are absent — which changes nothing about the order of the steps that
 * remain.
 */
...
/**
 * Each funnel's steps, in order, in this service's step vocabulary.
 *
...
/** The ordered steps a funnel key names, or null when it names no funnel this service knows. */
...
export function funnelStepIndexOf(
...
export function stepAndLater(
  ...
  const i = funnelStepIndexOf(funnelSteps, step);
