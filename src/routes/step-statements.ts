...
import {
  FunnelResolutionError,
  resolveCampaignFunnel,
...
/**
 * WHICH funnel this lead is on. A funnel is ORDERED, and "before" / "after" mean nothing without
 ...
 *   409 — the campaign states no funnel this service has steps for (or campaign-service does not
 *         know the campaign). Nothing is broken; there is simply no order to read the steps in, and
 *         a made-up order would print a sequence nobody stated.
...
async function resolveFunnelOrRespond(
  ...
  try {
    return await resolveCampaignFunnel(row.campaign_id, {
      orgId: req.orgId!,
    ...
    if (!(error instanceof FunnelResolutionError)) throw error;
    console.error(error.message);
    if (error.reason === "unavailable") {
      res.status(502).json({
        error:
          "campaign-service could not say which sales funnel this lead's campaign sells through, " +
          "so the order of its steps is unknown. No answer is returned rather than one built on " +
      ...
      error:
        error.reason === "unknown"
          ...
          : "This lead's campaign states no sales funnel, so its steps have no order. Declare the " +
            "campaign's funnel and its steps resolve.",
...
router.post(
  ...
  wrap(async (req: AuthenticatedRequest, res: Response) => {
    ...
    //
    // The platform automates the first leg of a sales funnel; the customer performs the rest —
    // they run the meeting, they close the deal — so they are the only one who knows what that
    // leg cost. Without it a funnel's cost of acquisition counts only the leg we billed for, and
    ...
    if (!resolved) return;
    const funnelSteps = resolved.funnelSteps;
    ...
    if (body.kind === "never") {
      ...
      const blockedBy = stepAndLater(funnelSteps, step);
    ...
    const supersedes = stepAndEarlier(funnelSteps, step);
 ...
 * read back what it stated — with the funnel's two rules already applied, because a funnel is
 * ORDERED and no two surfaces may show a lead as dead at one step and alive at a later one.
 *
 ...
 *
 * `funnelSteps` is this campaign's funnel, in order — per FUNNEL, never a universal step order.
 * Steps outside it (`inFunnel: false`) read from statements alone: no funnel rule reaches them.
 */
router.get(
  "/orgs/leads/:id/step-statements",
  apiKeyAuth,
 ...
 *
 * `?implied=true` additionally applies the funnel's order: a lead that will never book has, by the same
    ...
    try {
      ...
      res.status(502).json({
        error:
          ...
          "so no step order can be applied. No answer is returned rather than one built on an order " +
          "nobody stated.",
        code: "campaign_service_unavailable",
      });
      return;
    ...
    if (unstated.length > 0) {
      res.status(409).json({
        error:
          ...
          "no order and no implication can be applied. Declare those campaigns' funnels and it " +
          "resolves.",
 ...
 * acquisition could only count the leg the platform paid for, so every return computed for that
